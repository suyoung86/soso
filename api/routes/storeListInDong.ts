...
router.get("/", async (req: Request, res: Response) => {
  ...
  const apiUrl = "http://apis.data.go.kr/B553077/api/open/sdsc2/storeListInDong";

  try {
    const response = await axios.get(apiUrl, {
      params: { servicekey, divId, key, indsLclsCd, indsMclsCd, indsSclsCd, numOfRows, pageNo, type },
      responseType: "text"
    });
