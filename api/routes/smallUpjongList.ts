...
router.get("/", async (req: Request, res: Response) => {
  ...
  if (!resId || !catId || !ctprvnCd || !serviceKey || !type) {
    return res.status(400).json({ message: "Missing required query params" });
  }

  const apiUrl = "http://apis.data.go.kr/B553077/api/open/sdsc2/smallUpjongList";

  try {
    const response = await axios.get(apiUrl, {
      params: { resId, catId, ctprvnCd, serviceKey, type },
      responseType: "text"
    });
