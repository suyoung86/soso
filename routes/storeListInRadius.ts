...
router.get("/", async (req: Request, res: Response) => {
  ...
  const apiUrl = "http://apis.data.go.kr/B553077/api/open/sdsc2/storeListInRadius";

  try {
    const response = await axios.get(apiUrl, {
      params: { servicekey, radius, cx, cy, indsLclsCd, indsMclsCd, indsSclsCd, numOfRows, pageNo, type },
      responseType: "text"
    });
